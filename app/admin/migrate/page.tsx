        ...
        <div className="p-8">
          {status === 'idle' && (
            <div className="text-center">
              ...
              <button
                ...
                className="w-full py-4 bg-brand-600 hover:bg-brand-700 text-white font-extrabold rounded-2xl transition-all shadow-lg active:scale-[0.98]"
              >
                마이그레이션 시작하기
              </button>
            </div>
          ...
          {status === 'running' && (
            <div className="text-center py-10">
              ...
              <p className="text-foreground font-bold text-lg">마이그레이션 진행 중...</p>
              <p className="text-muted text-sm mt-2">창을 닫지 말고 잠시만 기다려 주세요.</p>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold text-foreground mb-6">마이그레이션 완료!</h2>
              
              <div className="bg-canvas rounded-2xl p-6 text-left space-y-3 mb-8">
                <div className="flex justify-between text-sm">
                  <span className="text-muted">소설 표지 이전</span>
                  <span className="font-bold text-brand-600">{result?.novels}개</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted">회차 메인 삽화 이전</span>
                  <span className="font-bold text-brand-600">{result?.episodes}개</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted">본문 내 삽화 이전</span>
                  <span className="font-bold text-brand-600">{result?.contentImages}건</span>
                </div>
              </div>
          ...
          {status === 'error' && (
            <div className="text-center">
              ...
              <button
                ...
                className="w-full py-4 bg-gray-100 text-muted font-extrabold rounded-2xl"
              >
                다시 시도하기
              </button>
            </div>
